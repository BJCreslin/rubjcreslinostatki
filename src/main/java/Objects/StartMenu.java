...
public class StartMenu implements ActionMenu {
    ...
    private HashMap<Item, Integer> fillMakeFileMap() {
        ...
        dublicatecode(makeFileMap, itemArrayList, vystavkaMap, centralMap, percentToMake);
        return makeFileMap;
    }

    public static void dublicatecode(HashMap<Item, Integer> makeFileMap, ArrayList<Item> itemArrayList, HashMap<Integer, Integer> vystavkaMap, HashMap<Integer, Integer> centralMap, double percentToMake) {
        for (Item item : itemArrayList) {
            if (item.isMake()) {
                ...
                if (itemOnVystavka == null) {
                    itemOnVystavka = 0;
                }

                Integer itemNeed = item.getCount();
                System.out.println(percentToMake + "     " + itemNeed + "     " + itemOnCentral + "   " + itemOnVystavka);
